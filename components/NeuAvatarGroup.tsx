...
export const NeuAvatarGroup: React.FC<AvatarGroupProps> = ({ avatars, max = 4, size = 'md', className = '' }) => {
...
<div className="rounded-full ring-4 ring-gray-100">
<NeuAvatar src={avatar.src} alt={avatar.alt} fallback={avatar.fallback} size={size}
...
<div className={`absolute inset-0 rounded-full pointer-events-none mix-blend-multiply opacity-20`} />
</div>
</div>
))}
{remaining > 0 && (
...
<div className="rounded-full ring-4 ring-gray-100">
<div className={`
${sizeClasses[size]} rounded-full bg-gray-100 flex items-center justify-center text-gray-500 font-bold border border-white/20
`} style={neuStyle('flat')}>
+{remaining}
</div>
</div>
