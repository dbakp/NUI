...
export const NeuCombobox: React.FC<ComboboxProps> = ({ 
  options, 
  value, 
  onChange, 
    ...
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      <div 
        ...
        className={`
            ...
            ${isOpen 
                ? 'shadow-neu-pressed text-neu-accent ring-1 ring-neu-accent/10' 
                : 'shadow-neu-flat bg-neu-base hover:shadow-neu-convex cursor-pointer'}
        `}
      >
        <div className="flex items-center gap-3 flex-1 px-4 py-3 min-h-[48px] overflow-hidden">
             {icon && (
                 <span className={`${isOpen ? 'text-neu-accent' : 'text-gray-400'}`}>
                     {icon}
                 </span>
             )}
        ...
        >
            <ChevronsUpDown size={16} className={`transition-transform duration-200 ${isOpen ? 'text-neu-accent' : ''}`} />
        </button>
      </div>
