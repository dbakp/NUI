...
export const NeuCreditCard: React.FC<CreditCardProps> = ({ 
    cardNumber, 
    cardHolder, 
    expiryDate, 
    cardType = 'visa', 
    ...
    return (
        <div className={`
            relative overflow-hidden rounded-[2rem] p-8 w-full max-w-[380px] aspect-[1.586/1] flex flex-col justify-between
            shadow-neu-convex transition-transform duration-300 hover:scale-[1.02]
            ${variant === 'dark' ? 'bg-gray-800 text-white' : 'bg-neu-base text-gray-700'}
            ${className}
        `}>
            {/* Background Decor */}
            <div className={`
                absolute -right-12 -top-12 w-64 h-64 rounded-full opacity-10
                ${variant === 'dark' ? 'bg-white' : 'bg-neu-accent'}
            `} />
