...
export const NeuLabelPicker: React.FC<LabelPickerProps> = ({ labels, selectedIds, onChange, className = '' }) => {
...
<NeuButton onClick={() => setIsOpen(!isOpen)} variant="secondary" size="sm" icon={<Tag size={14}/>}
isActive={isOpen || selectedIds.length > 0}
>
{selectedIds.length > 0 ? `${selectedIds.length} Labels` : 'Labels'}
</NeuButton>
{isOpen && (
<div className="absolute top-full left-0 mt-3 p-3 w-56 z-20
...
${isSelected ? 'bg-gray-100' : 'hover:bg-white/40 hover:'}
`}
>
