...
export const NeuOccupancyHeatmap: React.FC<OccupancyHeatmapProps> = ({ data, className = '' }) => {
const hours = Array.from({length: 10}, (_, i) => i + 8); // 8 AM to 5 PM
const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'];
const getIntensityColor = (value: number) => {
if (value === 0) return 'bg-gray-200/30';
if (value < 30) return 'bg-indigo-500/20';
if (value < 60) return 'bg-indigo-500/50';
...
<span>Low</span>
<div className="w-20 h-2 rounded-full bg-gradient-to-r from-neu-accent/10 to-neu-accent"></div>
<span>High</span>
</div>
</div>
...
>
<div className={`
w-full h-full rounded-lg transition-all duration-300 border border-transparent hover:border-white/50 hover:scale-110 hover:shadow-lg hover:z-10 relative
${getIntensityColor(val)}
`}
/>
{/* Tooltip */}
<div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-800 text-white text-[10px] font-bold rounded-md opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-20
">
{val}%
</div>
</div>
