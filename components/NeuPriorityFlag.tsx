...
export const NeuPriorityFlag: React.FC<PriorityFlagProps> = ({ priority, onChange, className = '' }) => {
const priorities = [
...
${priority === p.value ? 'bg-gray-100 scale-110' : 'hover:bg-white/40 opacity-50 hover:opacity-100'}
`}
title={`Priority ${p.value}`}
>
<Flag size={16} className={p.color} fill={priority === p.value && p.value !== 4 ? 'currentColor' : 'none'}
/>
</button>
))}
</div>
