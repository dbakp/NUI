...
export const NeuRoomStatusPanel: React.FC<RoomDisplayProps> = ({ roomName, status, currentMeeting, nextMeeting, onBookNow, className = '' }) => {
const isBusy = status === 'busy';
return (
<div className={`
relative overflow-hidden rounded-[3rem] bg-gray-100 border-[8px] border-gray-100 flex flex-col h-[500px] w-full max-w-sm mx-auto
${className}
`} style={neuStyle('flat')}>
{/* Status Header */}
<div className={`
flex-1 p-8 flex flex-col justify-between relative z-10 transition-colors duration-500
${isBusy ? 'bg-red-500/10' : 'bg-emerald-500/10'}
`}>
<div className="flex justify-between items-start">
<h2 className="text-3xl font-extrabold text-gray-800">{roomName}</h2>
<div className={`
px-4 py-2 rounded-xl text-sm font-bold uppercase tracking-widest border border-white/20
${isBusy ? 'bg-red-500 text-white' : 'bg-emerald-500 text-white'}
`}>
...
<p className="text-xs font-bold text-red-500 uppercase tracking-widest mb-2">Current Meeting</p>
...
<button
onClick={onBookNow}
className="w-full py-4 rounded-2xl font-bold text-lg active:scale-[0.98]
bg-gray-100 text-gray-700 border border-white/20
