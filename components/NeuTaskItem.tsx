...
export const NeuTaskItem: React.FC<TaskItemProps> = ({ 
    id, 
    title, 
    completed, 
    onToggle, 
    ...
    const priorityColors = {
        low: 'bg-neu-success',
        medium: 'bg-neu-warning',
        high: 'bg-neu-danger'
    };

    return (
        <div className={`
            flex items-center gap-4 p-4 rounded-2xl bg-neu-base 
            transition-all duration-300 border border-white/20
            ${completed ? 'opacity-60 shadow-neu-pressed' : 'shadow-neu-flat hover:shadow-neu-convex'}
            ${className}
        `}>
            <button
                onClick={() => onToggle(id)}
                className={`
                    ...
                    ${completed ? 'bg-neu-accent shadow-neu-pressed text-white' : 'bg-neu-base shadow-neu-pressed text-transparent hover:text-gray-300'}
                `}
            >
