...
${hasChildren ? 'text-gray-500 hover:text-indigo-500' : 'opacity-0'}
`}>
<ChevronRight size={14} className={`transition-transform duration-200 ${isOpen ? 'rotate-90' : ''}`} />
</div>
{/* Icon */}
<div className={`
${hasChildren ? 'text-indigo-500' : 'text-gray-400'}
`}>
...
return (
<div className={`
bg-gray-100 rounded-[2rem] p-4 border border-white/20 overflow-hidden
${className}
`} style={neuStyle('flat')}>
{items.map(item => (
